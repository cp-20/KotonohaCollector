...
if (listener != null)
{
listener!!.onMove(resolvedDirection())
}
postDelayed(this, CursorRepeatPolicy.intervalMs())
}
}
...
"カーソルを右へ。上下左右にフリック、長押しで連続移動"
else
"カーソルを左へ。上下左右にフリック、長押しで連続移動")
}
...
showGuide()
postDelayed(repeatMove, CursorRepeatPolicy.initialDelayMs(
...
removeCallbacks(repeatMove)
dismissGuide()
super.onDetachedFromWindow()
}

private fun resolvedDirection():Direction {
return if (direction == null || direction == Direction.CENTER)
if (defaultRight) Direction.RIGHT else Direction.LEFT
else
direction!!
}
