...
internal class MozcConversionEngine private constructor(context: Context) : ConversionEngine {
    ...
    private fun readCandidates(output: ProtoCommands.Output, reading: String): List<String> {
        ...
        if (output.hasAllCandidateWords()) {
            output.allCandidateWords.candidatesList.forEach { candidate ->
                addCandidate(values, ids, candidate.value, candidate.id.takeIf { candidate.hasId() })
            }
        }
        ...
        addCandidate(values, ids, reading, null)
        addCandidate(values, ids, reading.toKatakana(), null)
        candidateIds.clear()
    ...
    companion object {
        ...
        private fun addCandidate(
            ...
            if (!value.isNullOrEmpty() && values.add(value)) ids.add(id ?: FALLBACK_CANDIDATE_ID)
        }
